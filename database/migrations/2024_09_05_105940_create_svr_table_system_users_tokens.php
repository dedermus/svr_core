    ...
    public function up(): void
    {
        ...
        if (!Schema::hasTable('system.system_users_tokens'))
        {
            Schema::create('system.system_users_tokens', function (Blueprint $table) {
                ...
                $table->string('browser_name', 32)->nullable(false)->comment('Название браузера');
                $table->string('browser_version', 32)->nullable(false)->comment('Версия браузера');
                $table->string('platform_name', 32)->nullable(false)->comment('Имя платформы');
                $table->string('platform_version', 32)->nullable(false)->comment('Версия платформы');
                ...
                $table->timestamp('token_created_at')->nullable(false)->default(DB::raw('CURRENT_TIMESTAMP'))->comment('Дата создания записи');
