...
use packages\svr\core\src\Traits\PostgresGrammar;
    ...
    public function up(): void
    {
        ...
        if (!Schema::hasTable('system.system_users'))
        {
            Schema::create('system.system_users', function (Blueprint $table) {
                ...
                $table->string('user_guid', 64)->nullable(false)->comment('UUID4');
                $table->string('user_first', 32)->nullable(false)->comment('Имя');
                $table->string('user_middle', 32)->nullable(false)->comment('Отчество');
                $table->string('user_last', 32)->nullable(false)->comment('Фамилия');
                $table->string('user_avatar', 255)->nullable(false)->comment('Иконка (аватар)');
                $table->string('user_password', 64)->nullable(false)->comment('Пароль');
                $table->string('user_sex', 255)->nullable(false)->comment('Пол (гендерная принадлежность)');
                ...
                $table->string('user_phone', 15)->nullable(false)->comment('Телефон');
                ...
                $table->timestamp('user_created_at')->nullable(false)->default(DB::raw('CURRENT_TIMESTAMP'))->comment('Дата создания записи');
                ...
                $table->timestamp('user_date_update')->nullable(false)->default(DB::raw('CURRENT_TIMESTAMP'))->comment('Дата обновления');
